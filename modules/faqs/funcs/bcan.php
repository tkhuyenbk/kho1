...
$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM `" . NV_PREFIXLANG . "_" . $module_name . "_question` WHERE status != 0 AND userid = ".$user_info['userid']." AND most !=0 ORDER BY `addtime` DESC LIMIT " . $page . ", " . $per_page;
$result = $db->query( $sql );
...
if ( ! empty( $array ) )
{
 ...
 $xtpl->assign( 'que', $lang_module['bcan'] );
